...
def call_llm(
    ...
    # JSON 模式（并非所有模型都支持，但 GPT-4o 系列支持）
    if json_mode:
        kwargs["response_format"] = {"type": "json_object"}

    # 对于推理模型（如 Kimi-K2.5），关闭 thinking 模式
    if llm_cfg.disable_thinking:
        kwargs["extra_body"] = {"thinking": {"type": "disabled"}}

    log.info(f"LLM 调用: tier={tier}, model={model}, tokens_limit={max_tokens}")
    ...
    try:
        ...
        if usage:
            log.debug(
                f"LLM 用量: prompt={usage.prompt_tokens}, "
        ...
        log.error(f"LLM 调用失败 (tier={tier}, model={model}): {e}")
        raise
