...
class RunTracker:
    ...
    def stage_end(self, name: str, status: str = "success", error: str | None = None):
        for stage in self.stages:
            if stage["name"] == name and stage["status"] == "running":
                ...
                stage["status"] = status
                stage["error"] = error
                elapsed = (stage["end"] - stage["start"]).total_seconds()
                if status == "success":
                    self.log.info(f"✅ 阶段完成: {name} ({elapsed:.1f}s)")
                else:
                    self.log.error(f"❌ 阶段失败: {name} ({elapsed:.1f}s) — {error}")
                break
    ...
    def summary(self) -> str:
        ...
        for stage in self.stages:
            elapsed = ""
            if stage["end"] and stage["start"]:
                elapsed = f" ({(stage['end'] - stage['start']).total_seconds():.1f}s)"
            icon = "✅" if stage["status"] == "success" else "❌"
            lines.append(f"  {icon} {stage['name']}{elapsed}")
            if stage["error"]:
