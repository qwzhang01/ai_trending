...
def _make_github_response(
    status_code=200,
    html_url="https://github.com/owner/repo/blob/main/reports/2026-03-19.md",
):
    mock = MagicMock()
...
class TestGitHubPublishInput:
    ...
    def test_custom_values(self):
        inp = GitHubPublishInput(
            content="x", filename="custom.md", commit_message="feat: add report"
        )
...
class TestSaveLocally:
    def test_saves_file_to_reports_dir(self, tool, tmp_output_dir):
        tool._save_locally(SAMPLE_CONTENT, "test.md", "测试原因")
    ...
    def test_auto_generates_filename_from_date(self, tool, tmp_output_dir):
        tool._save_locally(SAMPLE_CONTENT, "", "测试原因")
        reports_dir = tmp_output_dir / "reports"
        # 应该有一个以日期命名的文件
    ...
    def test_returns_warning_message(self, tool, tmp_output_dir):
        result = tool._save_locally(
            SAMPLE_CONTENT, "test.md", "未设置 GITHUB_TRENDING_TOKEN"
        )
...
class TestGitHubPublishToolRunFallback:
    def test_no_token_saves_locally(self, tool, tmp_output_dir):
        ...
        from ai_trending.config import (
            AppConfig,
            GitHubConfig,
            LLMConfig,
            NewsConfig,
        ...
        with patch(
            "ai_trending.tools.github_publish_tool.load_config", return_value=fake_cfg
        ):
...
class TestGitHubPublishToolRunSuccess:
    def test_push_new_file(self, tool, tmp_output_dir, github_env):
        ...
        with patch(
            "ai_trending.tools.github_publish_tool.safe_request",
    ...
    def test_push_failure_falls_back_to_local(self, tool, tmp_output_dir, github_env):
        ...
        with patch(
            "ai_trending.tools.github_publish_tool.safe_request", return_value=None
        ):
            result = tool._run(content=SAMPLE_CONTENT)
        assert "本地路径" in result
        assert "⚠️" in result
